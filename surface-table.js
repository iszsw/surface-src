...
;(function (global, factory) {
    ...
    const editableComponent = function () {
        ...
        return {
            ...
            props: ["$index", "column", "type", "row", "options", "async"],
            created() {
                this.reload()
            },
            watch: {
        ...
        return {
            ...
            methods: {
                ...
                sizeChange(pageSize) {
                    ...
                    if (Math.floor(this.options.props.total / pageSize) > this.currentPage) this.load()
                },
                load(params = {}) {
                        ...
                        if (bindThisAttributes.indexOf(x) >= 0) {
                            if (isFunction(original[x])) {
                                ...
                                for (let i in original[x]) {
                                    if (isFunction(original[i])) {
                                        original[i] = original[i].bind(bindObj)
                                    }
                                }
                            }
                        }
                ...
                value: function _initChildren(children) {
                    ...
                    if (Array.isArray(children)) {
                        children = children.reduce(function (children, c) {
                            isEmpty(c) || children.push(isObject(c) ? new Render({...c}, vm).run() : c)
                            return children
                        }, [])
                    }
                    return children
